...
export class ShishaWorldScraper implements ShishaShopScraper {
  async #loadTobaccosFromProducer(
    ...
    try {
      ...
      return <TobaccosWithProducer>products.map((link) => {
        const source = $(link).attr('href');
        return <Partial<TobaccoWithProducer>>{
  ...
  async #loadTobaccoDetails(
    tobacco: TobaccoWithProducer
  ): Promise<TobaccoWithProducer> {
    try {
      const detailRequestOptions = {
        ...ShishaWorldRequestOptions,
        url: `${tobacco.source.replace(
          ShishaWorldRequestOptions.baseURL!,
          ''
        )}`,
        params: {},
      };
